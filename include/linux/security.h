/*
 * Linux Security plug - Minimal stub implementation
 *
 * All security functionality has been removed.
 * This provides stub implementations for compatibility.
 */
...
enum lsm_integrity_type {
	...
	LSM_INT_IMA_MEASUREMENT,
	LSM_INT_IMA_APPRAISE_DIGSIG,
	LSM_INT_IMA_APPRAISE_MODSIG,
};
...
/* All security functions are now stubs that do nothing */
...
static inline int security_sb_kern_mount(const struct super_block *sb)
{
	return 0;
}
...
static inline int security_kernel_act_as(struct cred *new, u32 secid)
{
	return 0;
}

static inline int security_kernel_create_files_as(struct cred *new,
						   struct inode *inode)
{
	return 0;
}
...
static inline int security_setprocattr(int lsmid, const char *name,
...
static inline void security_sk_classify(struct sock *sk, struct flowi_common *flic)
{
}
